...
class PipelinePlant(BaseModel):
    ...
    # Rough estimates, wind only — two separate phases, not one combined
    # number, since one is temporary and the other permanent. See
    # CapacityPipeline.jobs_estimate_note.
    estimated_construction_jobs: float | None
    estimated_operation_jobs: float | None
...
class PipelineZoneSummary(BaseModel):
    ...
    n_plants: int
    estimated_construction_jobs: float  # sum for this zone (0 if none/hydro-only)
    estimated_operation_jobs: float  # sum for this zone (0 if none/hydro-only)
...
class CapacityPipeline(BaseModel):
    ...
    national_estimated_construction_jobs: float  # sum over ALL plants, incl. zone=NULL ones
    national_estimated_operation_jobs: float  # sum over ALL plants, incl. zone=NULL ones
