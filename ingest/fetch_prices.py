#!/usr/bin/env python3
"""
Fetch day-ahead spot prices for NO1-NO5 and selected European zones from
ENTSO-E, and store them in TimescaleDB (if DATABASE_URL is set) and/or a
local CSV file under ingest/output/.

Usage:
    python fetch_prices.py                # last 7 days, all default zones
    python fetch_prices.py --days 14
    python fetch_prices.py --zones NO1 NO2 DE_LU
"""
...
def fetch_all(client: EntsoeClient, zones: list[str], period_start: datetime, period_end: datetime) -> list[PricePoint]:
    all_points: list[PricePoint] = []
    for zone in zones:
        eic = ZONES[zone]
        try:
            points = client.get_day_ahead_prices(zone, eic, period_start, period_end)
            print(f"  {zone}: {len(points)} price points")
...
def write_csv(points: list[PricePoint], path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["zone", "timestamp_utc", "price_eur_mwh", "resolution_min"])
        for p in sorted(points, key=lambda p: (p.zone, p.timestamp_utc)):
            writer.writerow([p.zone, p.timestamp_utc.isoformat(), p.price_eur_mwh, p.resolution_min])
    print(f"Wrote {len(points)} rows to {path}")


def write_db(points: list[PricePoint], database_url: str) -> None:
    import psycopg2
    ...
    try:
        with conn, conn.cursor() as cur:
            execute_values(
                cur,
                """
                INSERT INTO spot_price (zone, timestamp_utc, price_eur_mwh, resolution_min, source)
                VALUES %s
                ...
                """,
                [(p.zone, p.timestamp_utc, p.price_eur_mwh, p.resolution_min, "entsoe") for p in points],
            )
        print(f"Upserted {len(points)} rows into spot_price")
    finally:
        conn.close()
...
def main() -> int:
    ...
    parser.add_argument("--csv", default=str(OUTPUT_DIR / "prices.csv"), help="Path to write CSV output")
